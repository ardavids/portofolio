...
export default function Experience() {
  const experiences = [
    {
      role: "Fullstack Developer (AI-Assisted)",
      company: "Independent Project",
      date: "Mar 2026 – Sekarang",
      location: "Remote",
      type: "Self-Project",
      desc: "Membangun SaaS 'CatatAja' menggunakan workflow pengembangan berbasis AI yang efisien. Merancang arsitektur database di Supabase, mengelola state menggunakan Zustand, serta fokus pada optimasi performa dan respon UI di Next.js.",
      skills: ["Next.js", "Supabase", "AI Dev Flow", "Fullstack"],
      color: "violet",
    },
    {
  ...
  const colorMap: Record<
    string,
    { border: string; dot: string; badge: string; tag: string }
  > = {
    violet: {
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute left-6 md:left-1/2 top-6 transform -translate-x-1/2 hidden md:flex items-center justify-center">
                    <div
                      className={`w-4 h-4 rounded-full ${c.dot} ring-4 ring-slate-900`}
                    />
                  </div>
                  ...
                  <div className="flex-1">
                    <div
                      className={`glass glass-hover rounded-2xl p-6 border ${c.border} group`}
                    >
                      {/* Top row */}
                      <div className="flex justify-between items-start flex-wrap gap-3 mb-4">
                        <div>
                          ...
                          <p
                            className={`text-sm font-semibold mt-0.5 ${exp.color === "violet" ? "text-violet-400" : "text-cyan-400"}`}
                          >
                            {exp.company}
                          </p>
                        </div>
                        <span
                          className={`text-xs font-semibold px-3 py-1 rounded-full border ${c.badge}`}
                        >
                          {exp.type}
                        </span>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {exp.skills.map((skill) => (
                          <span
                            key={skill}
                            className={`text-xs px-2.5 py-1 rounded-md font-medium ${c.tag}`}
                          >
                            {skill}
                          </span>
                        ))}
